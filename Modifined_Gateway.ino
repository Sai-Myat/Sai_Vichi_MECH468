...
String myWriteAPIString = "QLDMZEUTF679Z6OK";
uint16_t crcdata = 0;
    ...
    if (rf95.recv(buf, &len))//Check if there is incoming data
    {
      ...
      { 
        if(buf[0] == 1||buf[1] == 1||buf[2] ==1) //Check if the ID match the LoRa Node ID
        {
          ...
          rf95.waitPacketSent();
          int newData[8] = {0, 0, 0, 0, 0, 0, 0, 0}; //Store Sensor Data here<<<<<<<<<<<<<Might need to change here
          for (int i = 0; i < 8; i++)
          {
            newData[i] = buf[i + 3];
          }
          ...
          Console.print(".");
          Console.println(hl);
          //Show Sound Detection (dB)
          Console.print("Get Sound (dB)");
          Console.print(ss);
          Console.print(".");
          Console.println(ts);
          //Show UV Detection
